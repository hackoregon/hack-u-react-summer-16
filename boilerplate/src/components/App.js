...
const API_INDV =
  'http://54.213.83.132/hackoregon/http/oregon_individual_contributors/';
const API_BIZ  =
...
export default class App extends Component {
  ...
  render() {
    ...
    const onRefreshForKey = key => async url => {
      ...
      this.setState({ [key]: { ...this.state[key], items: data } });
    };

    const onUpdateSortForKey = key => order => {
      this.setState({ [key]: { ...this.state[key], sortOrder: order } });
    };
